...
contract accessAuth is Ownable() {
    ...
    /** 
       @notice Updates a user's capability list (after a payment) adding the new purchased resources to the list.
			   The ETH tokens are directly transfered to the owner of the contract.
			   The capability list is a list of strings where a string identifies a resource
       @param _resources List of strings
    */
    function buyResources(string[] memory _resources)
        external 
        payable 
    {
     ...
     */
    function updateKDS_Hash(bytes32 _newKDS_Hash) 
        onlyOwner() 
        external
    {
    ...
    function getCapabilityListByAddress(address _buyer) 
        external 
        view 
